...
function md5(input: string): string {
  // 这是简化的 MD5 演示，实际生产环境应使用 crypto-js 或 Web Crypto API
  let hash = 0;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={handleHash} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
          <FileText className="w-4 h-4" />计算 MD5
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="输入要计算 MD5 的文本..." className="w-full h-96 p-4 bg-gray-50 border border-gray-200 rounded-lg font-mono text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">MD5 结果</label>
          <div className="w-full h-96 p-4 bg-gray-50 border border-gray-200 rounded-lg">
            {output ? (
              <div className="font-mono text-lg text-gray-900 break-all">{output}</div>
            ) : (
              <span className="text-gray-400 text-sm">MD5 哈希值...</span>
            )}
          </div>
        </div>
      </div>
